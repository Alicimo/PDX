...
rnaseq.subsets <- lapply(rnaseq.subsets, function(ds){
  ...
  i <- rowSums(cpm(ds$human$data) > 1) >= 3
  x <- ds$human$data[i,]
  x <- DGEList(x)
  x <- calcNormFactors(x)
  ...
  des <- model.matrix(~0+batch+AR.stat)
...
ps <- lapply(names(rnaseq.subsets), function(dsname){
  ds <- rnaseq.subsets[[dsname]]
  AR <- cbind(ds$meta,"AR"=ds$E.cor[which(AR$ensembl_gene_id == rownames(ds$E.cor)),])
  (p <- ggplot(AR) + aes(x=factor(AR.status),y=AR) + geom_boxplot() + theme_tufte(20) + labs(y="AR Expression",x="AR IHC",title=dsname) + scale_x_discrete(labels=c("Negative","Positive","Unknown")))
})
grid.arrange(grobs=ps)
```
...
ds <- lapply(names(rnaseq.subsets), function(dsname){
  ds <- rnaseq.subsets[[dsname]]
  AR <- cbind(ds$meta,"AR"=ds$E.cor[which(AR$ensembl_gene_id == rownames(ds$E.cor)),])
  AR[,patient.sample:=as.character(interaction(patient,tissue.sampled))]
  
})
x <- merge(ds[[1]],ds[[2]],by="patient.sample")

ggplot(x) + aes(x=AR.x,AR.y) + geom_point() + labs(x=names(rnaseq.subsets)[1],y=names(rnaseq.subsets)[2],title="AR Expression")
```
...
grid.arrange(grobs=lapply(ps,"[[",4))
```
