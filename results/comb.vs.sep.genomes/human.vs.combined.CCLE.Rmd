...
data.dir <- paste0(project.dir,"data/human.vs.comb/CCLE/")
...
rnaseq$meta <- data.table(
  sname=basename(colnames(rnaseq$count)),
  genome=rep(sapply(dnames,basename),each=3),
  sample.ID=gsub(".star.bam","",basename(colnames(rnaseq$count))),
...
colnames(d) <- gsub("genomefeatureCounts.","",colnames(d))
