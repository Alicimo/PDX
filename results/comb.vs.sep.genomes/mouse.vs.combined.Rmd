...
data.dir <- paste0(project.dir,"data/mouse.vs.comb/")
dnames <- list.dirs(data.dir,recursive = F)
dnames <- dnames[!grepl("_archive",dnames)]
x <- lapply(dnames,function(dname){
  x <- lapply(list.files(dname,full.names = T,include.dirs = F),load.counts)
  x <- list(
    do.call("cbind",lapply(x,"[[","mouse")),
    do.call("cbind",lapply(x,"[[","human"))
  )
  for(i in 1:2) colnames(x[[i]]) <- paste0(basename(dname),".",colnames(x[[i]]))
  return(x)
})

rnaseq <- list()
rnaseq$count <- do.call("cbind",lapply(x,"[[",1))
...
Below we test for differences when one aligns mouse cell line rnaseq data against the combined (human + mouse) genome compared to the mouse genome alone. The below table indicates our experimental blanches/replicates:
...
* PC1 and PC2 accounts for 98% of the variance
...
if(nrow(top.genes)){
  ...
  print(top.genes <- merge(
    top.genes,
    biomaRt::select(org.Mm.eg.db,keys=top.genes$ENSEMBL,keytype = "ENSEMBL",columns = c("ENSEMBL","ENTREZID","SYMBOL"))
    ,sort=F
  ))
}
```
...
Below we perform a differential expression analysis between the different alignment branches. We find that use of the combined genome over mouse alone can lead to genes being found to be differentially expressed. Unsurprisingly, these genes were also found to contribute highly to PC3 in the above analysis.
...
DE.data <- DE.run(rnaseq$count,d,2,cont.matrix = cont)

for(i in 1:ncol(cont)){
  print(colnames(cont)[i])
  print(topTable.annotated.DE(DE.data$efit,spec="mouse",coef=i))
}

vennDiagram.paired(decideTest.annotated.DE(DE.data$efit,spec="mouse")[,2:(ncol(cont)+1)])
```